...
class DatabricksJobRunner:
    ...
    def run_notebook(self, notebook_path, timeout=600):
        ...
        try:
            ...
            job_payload = {
                "run_name": f"Retail-Pipeline-{int(time.time())}",
                "tasks": [
                    {
...
def trigger_real_pipeline():
    """
    Actually trigger the Databricks pipeline by running the orchestrator notebook
    """
    try:
        ...
        # Your actual notebook path in Databricks workspace
...
def get_real_pipeline_status(run_id):
    """
    Get actual status from Databricks job run
    """
    try:
...
@pipeline_bp.route('/run-single-notebook', methods=['POST'])
def run_single_notebook():
    """
    Run a specific notebook (for testing individual steps)
    """
    try:
        data = request.get_json()
        ...
        if not notebook_path:
            return jsonify({"status": "error", "message": "Notebook path required"}), 400
        ...
        if run_id:
            return jsonify({
                "status": "success", 
                "message": f"Notebook {notebook_path} execution started",
                "run_id": run_id
            })
        else:
            return jsonify({
                "status": "error",
                "message": f"Failed to start notebook: {notebook_path}"
            }), 500
